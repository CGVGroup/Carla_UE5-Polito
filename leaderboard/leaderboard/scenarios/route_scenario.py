...
class RouteScenario(BasicScenario):
    ...
    def _get_route(self, config):
        ...
        print("Interpolating route with {} keypoints".format(len(config.keypoints)))
        print("Route keypoints:", [[i.x, i.y, i.z] for i in config.keypoints])
        self.gps_route, self.route = interpolate_trajectory(config.keypoints)
        CarlaDataProvider.set_ego_route(self.route)
        # print("Route waypoints:", [[i[0].location.x, i[0].location.y, i[0].location.z,i[1]] for i in self.route])
