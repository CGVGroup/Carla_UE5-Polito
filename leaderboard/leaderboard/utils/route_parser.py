...
class RouteParser(object):
    ...
    def parse_routes_file(route_filename, routes_subset=''):
        ...
        for route in tree.iter("route"):
            ...
            failtriggers = []
            for position in route.find('failtriggers').iter('position'):
                failtriggers.append(carla.Location(x=float(position.attrib['x']),
                                                y=float(position.attrib['y']),
                                                z=float(position.attrib['z'])))
            route_config.failtriggers = failtriggers

            llmtriggers = []
            for position in route.find('llmtriggers').iter('position'):
                llmtriggers.append((carla.Location(x=float(position.attrib['x']),
                                                  y=float(position.attrib['y']),
                                                  z=float(position.attrib['z'])),position.attrib["text"]))
            route_config.llmtriggers = llmtriggers
