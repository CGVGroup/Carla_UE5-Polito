...
def validate_sensor_configuration(sensors, agent_track, selected_track):
    ...
    if Track(selected_track) != agent_track:
        print("WARNING: Agent track [{}] does not match the selected track [{}]!".format(
            Track(agent_track), Track(selected_track)))
...
class AgentWrapper(object):
    ...
    def set_failtriggers(self, failtriggers):
        """
        Set the failtriggers for the agent
        :param failtriggers: list of failtriggers
        """
        self._agent.set_failtriggers(failtriggers)

    def set_llmtriggers(self, llmtrigger):
        """
        Set the llm triggers for the agent
        :param llmtrigger: list of llm triggers
        """
        self._agent.set_llmtriggers(llmtrigger)
