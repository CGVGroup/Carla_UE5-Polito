...
class OppositeActorFlow(AtomicBehavior):
    ...
    def __init__(self, reference_wp, reference_actor, spawn_dist_interval,
             time_distance=1.5, base_distance=30, sink_dist=2, max_vehicles=None, name="OppositeActorFlow"):
        """
        Setup class members
        """
        super().__init__(name)
    ...
    def _move_waypoint_backwards(self, wp, distance):
        ...
        prev_wp = wp
        while dist < distance:
            print("Moving backwards from waypoint: ", prev_wp)
            prev_wps = prev_wp.previous(1)
            if prev_wps is None or len(prev_wps) == 0:
                print("No previous waypoint found, stopping backwards movement")
                break
    ...
    def initialise(self):
        ...
        print("flow distance: ", self._flow_distance, "m")
    ...
    def _spawn_actor(self):
        if self._max_vehicles is not None and self._spawned_vehicles >= self._max_vehicles:
            return  # Do not spawn more vehicles
    ...
    def update(self):
        ...
        if self._max_vehicles is not None and self._spawned_vehicles >= self._max_vehicles:
            # No more spawning, but keep controlling existing actors
            if len(self._actor_list) == 0:
                return py_trees.common.Status.SUCCESS  # All vehicles removed
            else:
                return py_trees.common.Status.RUNNING
