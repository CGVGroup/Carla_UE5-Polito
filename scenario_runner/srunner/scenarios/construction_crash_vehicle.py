...
class ConstructionObstacle(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False,
        ...
        self._trigger_distance = 4
        self._opposite_wait_duration = 1
...
class ConstructionObstacleTwoWays(ConstructionObstacle):
    ...
    def __init__(
        ...
        self._max_vehicles = get_value_parameter(config, 'max_vehicles', int, 1)
    ...
    def _create_behavior(self):
        ...
        """
        reference_wp = self._reference_waypoint.get_left_lane()
        if not reference_wp:
        ...
        reference_wp_opposite = reference_wp.get_left_lane()
        if self.route_mode:
            root.add_child(RemoveRoadLane(self._reference_waypoint))
            root.add_child(RemoveRoadLane(reference_wp_opposite))
            root.add_child(LeaveCrossingSpace(self._reference_waypoint))
        ...
        if self.route_mode:
            ...
            behavior.add_child(ChangeOppositeBehavior(active=False,spawn_dist = 1000000))
            behavior.add_child(OppositeActorFlow(reference_wp, self.ego_vehicles[0], self._opposite_interval, max_vehicles=self._max_vehicles))
        behavior.add_child(InTriggerDistanceToLocation(
            self.ego_vehicles[0], self._end_wp.transform.location, self._trigger_distance))
        ...
        if self.route_mode:
            ...
            root.add_child(ChangeOppositeBehavior(active=True,spawn_dist = None))
            root.add_child(ReAddRoadLane(0))
        for actor in self.other_actors:
            root.add_child(ActorDestroy(actor))
